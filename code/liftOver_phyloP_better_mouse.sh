#!/bin/bash
#SBATCH --job-name=mouse_phyloP
#SBATCH --output=outs/mouse_phyloP.out
#SBATCH --error=errs/mouse_phyloP.err
...
#     echo "liftOver"
#     liftOver -bedPlus=4 ${BIGWIG%%.*}".chr"$chr.bedGraph $liftOver_chainfile_path"mm10ToMm39.over.chain" ${BIGWIG%%.*}".chr"$chr.Mm39.bedGraph unMapped.chr$chr.${BIGWIG%%.*} 
#     echo "bedSort"
#     bedSort ${BIGWIG%%.*}".chr"$chr.Mm39.bedGraph ${BIGWIG%%.*}".chr"$chr.Mm39.sorted.bedGraph
# done
...
# for chr in {1..19} X Y; do
#   awk -v chr="$chr" -v base="${BIGWIG%%.*}" '{print > "separate_chroms/"base".chr"chr"_"$1".bedgraph"}' "${BIGWIG%%.*}.chr${chr}.Mm39.sorted.bedGraph"
# done
...
cat chr1_unique_sorted.bed chr2_unique_sorted.bed chr3_unique_sorted.bed chr4_unique_sorted.bed chr5_unique_sorted.bed \
     ...
     chr16_unique_sorted.bed chr17_unique_sorted.bed chr18_unique_sorted.bed chr19_unique_sorted.bed chrX_unique_sorted.bed chrY_unique_sorted.bed \
...
bedGraphToBigWig all_chromosomes_unique_sorted.bed ../mm39.chrom.sizes ../phyloP.mm39.bw
